-- Harden checkout pricing, voucher scope, and store-owner order isolation.
-- NOTE: This project has enum types created externally (voucher_scope,
-- order_status_type, payment_status_type). This migration converts relevant
-- columns back to TEXT for consistency with our schema.

-- Convert any externally-created enum columns back to TEXT
DO $$ BEGIN
  -- vouchers.scope: voucher_scope → TEXT
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'vouchers'
      AND column_name = 'scope'
      AND udt_name = 'voucher_scope'
  ) THEN
    ALTER TABLE public.vouchers ALTER COLUMN scope TYPE TEXT;
    ALTER TABLE public.vouchers ALTER COLUMN scope SET DEFAULT 'admin';
    ALTER TABLE public.vouchers ALTER COLUMN scope SET NOT NULL;
  END IF;
  -- order_status_history.from_status: order_status_type → TEXT
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'order_status_history'
      AND column_name = 'from_status'
      AND udt_name = 'order_status_type'
  ) THEN
    ALTER TABLE public.order_status_history ALTER COLUMN from_status TYPE TEXT;
  END IF;
  -- order_status_history.to_status: order_status_type → TEXT
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'order_status_history'
  ...
  END IF;
  -- payments.status: payment_status_type → TEXT
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'payments'
      AND column_name = 'status'
      AND udt_name = 'payment_status_type'
  ) THEN
...
ALTER TABLE public.vouchers ADD COLUMN IF NOT EXISTS scope TEXT NOT NULL DEFAULT 'admin';
ALTER TABLE public.vouchers ADD COLUMN IF NOT EXISTS seller_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE public.vouchers DROP CONSTRAINT IF EXISTS vouchers_scope_check;
UPDATE public.vouchers SET scope = 'admin' WHERE scope NOT IN ('admin', 'shop');
ALTER TABLE public.vouchers
  ADD CONSTRAINT vouchers_scope_check
  CHECK (scope IN ('admin', 'shop'));

CREATE INDEX IF NOT EXISTS idx_vouchers_scope_seller
  ON public.vouchers(scope, seller_id, is_active, start_date, end_date);

DROP POLICY IF EXISTS "Sellers manage own shop vouchers" ON public.vouchers;
CREATE POLICY "Sellers manage own shop vouchers"
  ON public.vouchers FOR ALL
  TO authenticated
  USING (scope = 'shop' AND seller_id = auth.uid())
  WITH CHECK (scope = 'shop' AND seller_id = auth.uid());

ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS seller_id UUID REFERENCES auth.users(id) ON DELETE SET NULL;
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS store_status TEXT NOT NULL DEFAULT 'pending';
ALTER TABLE public.order_items ADD COLUMN IF NOT EXISTS store_status_updated_at TIMESTAMPTZ;

ALTER TABLE public.order_items DROP CONSTRAINT IF EXISTS order_items_store_status_check;
ALTER TABLE public.order_items
  ADD CONSTRAINT order_items_store_status_check
  CHECK (store_status IN ('pending', 'confirmed', 'processing', 'shipped', 'delivered', 'cancelled', 'returned'));
...
BEGIN
  ...
  IF p_admin_voucher_id IS NOT NULL THEN
    SELECT * INTO v_admin_voucher
    FROM public.vouchers
    WHERE id = p_admin_voucher_id
    FOR UPDATE;

    IF v_admin_voucher.id IS NULL
      OR v_admin_voucher.scope <> 'admin'
      OR NOT v_admin_voucher.is_active
      OR NOW() < v_admin_voucher.start_date
      OR NOW() > v_admin_voucher.end_date
      OR (v_admin_voucher.usage_limit IS NOT NULL AND v_admin_voucher.used_count >= v_admin_voucher.usage_limit)
    THEN
      RAISE EXCEPTION 'Admin voucher is not available';
    END IF;
  END IF;

  IF p_shop_voucher_id IS NOT NULL THEN
    SELECT * INTO v_shop_voucher
    FROM public.vouchers
    WHERE id = p_shop_voucher_id
    FOR UPDATE;

    IF v_shop_voucher.id IS NULL
      OR v_shop_voucher.scope <> 'shop'
      OR v_shop_voucher.seller_id IS NULL
      OR NOT v_shop_voucher.is_active
      OR NOW() < v_shop_voucher.start_date
      OR NOW() > v_shop_voucher.end_date
      OR (v_shop_voucher.usage_limit IS NOT NULL AND v_shop_voucher.used_count >= v_shop_voucher.usage_limit)
    THEN
      RAISE EXCEPTION 'Shop voucher is not available';
    END IF;
  END IF;
  ...
  LOOP
    ...
    SELECT
      ...
      COALESCE(pv.stock, pv.quantity, 0),
      p.name,
      COALESCE(NULLIF(pv.name, ''), NULLIF(pv.sku, '')),
      pv.sku,
      COALESCE(
        pv.image_url,
        (
    ...
    WHERE pv.id = v_variant_id
      AND COALESCE(p.status::TEXT, 'active') IN ('active', 'out_of_stock')
    FOR UPDATE OF pv;
    ...
    IF v_current_stock < v_quantity THEN
      RAISE EXCEPTION 'Product % does not have enough stock', v_product_name;
    END IF;

    v_line_total := v_unit_price * v_quantity;
    v_subtotal := v_subtotal + v_line_total;

    IF v_shop_voucher.id IS NOT NULL AND v_shop_voucher.seller_id = v_seller_id THEN
      v_shop_subtotal := v_shop_subtotal + v_line_total;
    END IF;
    ...
    VALUES (
      ...
      v_variant_id,
      v_seller_id,
      COALESCE(v_product_name, 'Sản phẩm'),
      v_variant_name,
      v_sku,
      v_image_url,
      v_unit_price,
  ...
  IF v_admin_voucher.id IS NOT NULL THEN
    IF v_subtotal < COALESCE(v_admin_voucher.min_order_amount, 0) THEN
      RAISE EXCEPTION 'Order amount does not meet admin voucher minimum';
    END IF;

    v_admin_discount := CASE
      WHEN v_admin_voucher.discount_type = 'percentage'
        THEN v_subtotal * (v_admin_voucher.discount_value / 100)
      ELSE v_admin_voucher.discount_value
    END;

    IF v_admin_voucher.max_discount_amount IS NOT NULL THEN
      v_admin_discount := LEAST(v_admin_discount, v_admin_voucher.max_discount_amount);
    END IF;
  END IF;

  IF v_shop_voucher.id IS NOT NULL THEN
    IF v_shop_subtotal <= 0 THEN
      RAISE EXCEPTION 'Shop voucher does not match any checkout item';
    END IF;

    IF v_shop_subtotal < COALESCE(v_shop_voucher.min_order_amount, 0) THEN
      RAISE EXCEPTION 'Shop amount does not meet voucher minimum';
    END IF;

    v_shop_discount := CASE
      WHEN v_shop_voucher.discount_type = 'percentage'
        THEN v_shop_subtotal * (v_shop_voucher.discount_value / 100)
      ELSE v_shop_voucher.discount_value
    END;
  ...
  v_shipping_fee := CASE
    WHEN LOWER(COALESCE(p_address->>'city', p_address->>'province', '')) LIKE '%ho chi minh%'
      OR LOWER(COALESCE(p_address->>'city', p_address->>'province', '')) LIKE '%hcm%'
      THEN 20000
    ELSE 30000
  END;
  ...
  IF v_admin_voucher.id IS NOT NULL THEN
    ...
    WHERE id = v_admin_voucher.id;
  END IF;

  IF v_shop_voucher.id IS NOT NULL THEN
    UPDATE public.vouchers
    SET used_count = COALESCE(used_count, 0) + 1, updated_at = NOW()
    WHERE id = v_shop_voucher.id;
  END IF;

  IF array_length(v_cart_item_ids, 1) IS NOT NULL THEN
...
BEGIN
  ...
  VALUES (
    p_order_id,
    v_from_status::order_status_type,
    p_status::order_status_type,
    p_seller_id,
    'Store owner updated their items'
  );
  ...
  SET status = v_parent_status::order_status_type,
      updated_at = NOW(),
...
BEGIN
  ...
  FROM public.order_items
  WHERE seller_id = p_seller_id
    AND store_status <> 'cancelled';

  SELECT COALESCE(SUM(total_price), 0) INTO v_revenue
  FROM public.order_items
  WHERE seller_id = p_seller_id
    AND store_status = 'delivered';
